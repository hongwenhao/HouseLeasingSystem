...
public class AlipayServiceImpl implements AlipayService { // 支付宝支付与回调验签的实现类
    ...
    private String resolveTradeStatus(Map<String, String> params, String outTradeNo) {
        ...
        log.warn("支付宝同步回跳缺少交易状态（trade_status），开始主动查询交易状态，商户订单号（outTradeNo）={}", outTradeNo);
    ...
    private String queryTradeStatusByOutTradeNo(String outTradeNo) {
        ...
        try {
            ...
            if (response == null) {
                log.warn("支付宝交易查询返回空响应，商户订单号（outTradeNo）={}", outTradeNo);
                return null;
            }
            ...
            log.error("支付宝交易查询异常，将返回空状态并触发支付确认失败，商户订单号（outTradeNo）={}", outTradeNo, e);
            return null;
        }
    }
