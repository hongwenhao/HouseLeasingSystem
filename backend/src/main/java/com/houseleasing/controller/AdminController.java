...
public class AdminController { // 管理员专用控制器：用户、房源、订单、合同、统计等后台能力
    ...
    private static final Set<String> ORDER_STATUS_KEYWORDS = Set.of( // 定义不可变常量，统一管理业务规则和提示文案，避免魔法值散落
            "PENDING", "APPROVED", "SIGNED", "REJECTED", "CANCELLED", "COMPLETED" // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ); // 标记代码块或调用语句的边界，保证语法结构完整
    /**
    ...
    private static final Set<String> CONTRACT_STATUS_KEYWORDS = Set.of( // 定义不可变常量，统一管理业务规则和提示文案，避免魔法值散落
            "DRAFT", "PENDING_SIGN", "TENANT_SIGNED", "LANDLORD_SIGNED", "FULLY_SIGNED", "CANCELLED" // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ); // 标记代码块或调用语句的边界，保证语法结构完整
    /**
     * 后台房源状态白名单：
    ...
    private static final String CREDIT_RANGE_CASE_SQL = // 定义不可变常量，统一管理业务规则和提示文案，避免魔法值散落
            "CASE " + // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    "WHEN credit_score >= 90 THEN '" + CREDIT_RANGE_EXCELLENT + "' " + // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    "WHEN credit_score >= 70 THEN '" + CREDIT_RANGE_GOOD + "' " + // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    "WHEN credit_score >= 60 THEN '" + CREDIT_RANGE_NORMAL + "' " + // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    "ELSE '" + CREDIT_RANGE_LOW + "' END"; // 执行当前语句以推进业务流程，并为后续步骤准备数据
    ...
    public Result<PageResult<Order>> listAllOrders( // 声明成员字段或工具方法，为后续业务逻辑提供数据与能力
        ...
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<Order> pageObj = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.extension.plugins.pagination.Page<>(page, size); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Order> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        if (StringUtils.hasText(keyword)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            String trimmedKeyword = keyword.trim(); // 执行赋值操作，将计算结果保存到变量中供后续步骤使用
            String normalizedStatusKeyword = trimmedKeyword.toUpperCase(Locale.ROOT); // 执行赋值操作，将计算结果保存到变量中供后续步骤使用
            ...
            if (!hasExplicitStatusFilter && ORDER_STATUS_KEYWORDS.contains(normalizedStatusKeyword)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
                wrapper.eq(Order::getStatus, normalizedStatusKeyword); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            } else { // 进入另一条分支逻辑，处理前置条件不满足时的情况
                wrapper.like(Order::getOrderNo, trimmedKeyword); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            } // 标记代码块或调用语句的边界，保证语法结构完整
        } // 标记代码块或调用语句的边界，保证语法结构完整
        // 管理员新增状态下拉筛选：
        ...
        if (StringUtils.hasText(status)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            ...
            if (ORDER_STATUS_KEYWORDS.contains(normalizedStatus)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
                wrapper.eq(Order::getStatus, normalizedStatus); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            } // 标记代码块或调用语句的边界，保证语法结构完整
        } // 标记代码块或调用语句的边界，保证语法结构完整
        wrapper.orderByDesc(Order::getCreateTime); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        ...
        Set<Long> houseIds = records.stream() // 创建集合变量，用于承载后续查询与组装的数据
                .map(Order::getHouseId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .filter(id -> id != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .collect(Collectors.toSet()); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        Map<Long, House> houseMap = houseIds.isEmpty() // 创建集合变量，用于承载后续查询与组装的数据
                ? Map.of() // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        records.forEach(order -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
            if (order.getTenantId() != null) userIds.add(order.getTenantId()); // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            if (order.getLandlordId() != null) userIds.add(order.getLandlordId()); // 进行条件判断，仅在满足业务前提时才执行后续逻辑
        ...
        records.forEach(order -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
            order.setHouse(houseMap.get(order.getHouseId())); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            order.setTenant(sanitizeUser(userMap.get(order.getTenantId()))); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            order.setLandlord(sanitizeUser(userMap.get(order.getLandlordId()))); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        }); // 执行当前语句以推进业务流程，并为后续步骤准备数据
    ...
    public Result<PageResult<Contract>> listAllContracts( // 声明成员字段或工具方法，为后续业务逻辑提供数据与能力
        ...
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<Contract> pageObj = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.extension.plugins.pagination.Page<>(page, size); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Contract> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        // 合同搜索增强：
        ...
        if (StringUtils.hasText(keyword)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            String trimmedKeyword = keyword.trim(); // 执行赋值操作，将计算结果保存到变量中供后续步骤使用
            com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Order> orderNoWrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            ...
            Set<Long> matchedOrderIds = matchedOrders.stream() // 创建集合变量，用于承载后续查询与组装的数据
                    .map(Order::getId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    .filter(id -> id != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    .collect(Collectors.toSet()); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            wrapper.and(w -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        if (StringUtils.hasText(status)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            String normalizedStatus = status.trim().toUpperCase(Locale.ROOT); // 执行赋值操作，将计算结果保存到变量中供后续步骤使用
            if (CONTRACT_STATUS_KEYWORDS.contains(normalizedStatus)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
                wrapper.eq(Contract::getStatus, normalizedStatus); // 执行当前语句以推进业务流程，并为后续步骤准备数据
            } // 标记代码块或调用语句的边界，保证语法结构完整
        } // 标记代码块或调用语句的边界，保证语法结构完整
        wrapper.orderByDesc(Contract::getCreateTime); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<Contract> result = contractMapper.selectPage(pageObj, wrapper); // 执行赋值操作，将计算结果保存到变量中供后续步骤使用
        List<Contract> records = result.getRecords(); // 创建集合变量，用于承载后续查询与组装的数据
        // 批量查询关联信息，避免 N+1 查询
        Set<Long> houseIds = records.stream() // 创建集合变量，用于承载后续查询与组装的数据
                .map(Contract::getHouseId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .filter(id -> id != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .collect(Collectors.toSet()); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        Map<Long, House> houseMap = houseIds.isEmpty() // 创建集合变量，用于承载后续查询与组装的数据
                ? Map.of() // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                : mapById(houseMapper.selectBatchIds(houseIds), House::getId); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        Set<Long> userIds = new HashSet<>(); // 创建集合变量，用于承载后续查询与组装的数据
        records.forEach(contract -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        Set<Long> orderIds = records.stream() // 创建集合变量，用于承载后续查询与组装的数据
                .map(Contract::getOrderId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .filter(id -> id != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .collect(Collectors.toSet()); // 执行当前语句以推进业务流程，并为后续步骤准备数据
    ...
    private Contract findCancellableLatestContractForOrderByAdmin(Order order) { // 声明一个方法入口，约定参数和返回值以完成对应业务动作
        ...
        if ("FULLY_SIGNED".equals(latestContract.getStatus())) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            String contractIdentifier = StringUtils.hasText(latestContract.getContractNo()) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    ? latestContract.getContractNo() // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    private Contract findLatestContractByOrderId(Long orderId) { // 声明一个方法入口，约定参数和返回值以完成对应业务动作
        ...
        wrapper.eq(Contract::getOrderId, orderId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .orderByDesc(Contract::getCreateTime) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    public Result<Map<String, Object>> getStatistics() { // 获取后台首页统计
        ...
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Contract> pendingWrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
    ...
    public Result<List<Map<String, Object>>> getAreaStats() { // 统计不同城市的房源数量
        com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<House> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        wrapper.select("city AS city", "COUNT(*) AS count") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .isNotNull("city") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .ne("city", "") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .groupBy("city") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .orderByDesc("count") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    public Result<List<Map<String, Object>>> getPriceTrends() { // 统计近几个月平均租金趋势
        com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<House> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        wrapper.select("DATE_FORMAT(create_time, '%Y-%m') AS month", "ROUND(AVG(price), 2) AS avgPrice") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .groupBy("DATE_FORMAT(create_time, '%Y-%m')") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .orderByDesc("month") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        raw.stream() // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .sorted(Comparator.comparing(map -> YearMonth.parse(String.valueOf(map.get("month")), MONTH_FORMATTER))) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .forEach(item -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    public Result<List<Map<String, Object>>> getCreditDistribution() { // 统计用户信用分区间分布
        ...
        com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<User> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        wrapper.select(CREDIT_RANGE_CASE_SQL + " AS score_range", "COUNT(*) AS count") // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        buckets.forEach((range, count) -> { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
            Map<String, Object> row = new LinkedHashMap<>(); // 创建集合变量，用于承载后续查询与组装的数据
    ...
    public Result<PageResult<House>> listPendingHouses( // 声明成员字段或工具方法，为后续业务逻辑提供数据与能力
            ...
            @RequestParam(defaultValue = "10") int size, // 通过注解声明接口路由/权限/文档等元信息，让框架自动生效
            @RequestParam(required = false) String keyword) { // 通过注解声明接口路由/权限/文档等元信息，让框架自动生效
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<House> pageObj = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.extension.plugins.pagination.Page<>(page, size); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<House> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        wrapper.eq(House::getStatus, "OFFLINE"); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        if (StringUtils.hasText(keyword)) { // 进行条件判断，仅在满足业务前提时才执行后续逻辑
            wrapper.and(w -> w.like(House::getTitle, keyword) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                    .or().like(House::getCity, keyword) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    public Result<Void> auditHouse(@PathVariable Long id, // 声明成员字段或工具方法，为后续业务逻辑提供数据与能力
        ...
        sendHouseManagementNotificationToOwner(house, approved ? HOUSE_ACTION_ONLINE : HOUSE_ACTION_OFFLINE, // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    private BigDecimal parseDecimal(Object value) { // 声明一个方法入口，约定参数和返回值以完成对应业务动作
        ...
        } catch (Exception ignored) { // 该行用于衔接当前业务步骤，保证整体流程顺序执行
            return BigDecimal.ZERO; // 返回本次方法的处理结果给调用方，结束当前流程
        } // 标记代码块或调用语句的边界，保证语法结构完整
    } // 标记代码块或调用语句的边界，保证语法结构完整
    ...
    private <T> Map<Long, T> mapById(List<T> list, Function<T, Long> idGetter) { // 声明一个方法入口，约定参数和返回值以完成对应业务动作
        ...
        return list.stream() // 返回本次方法的处理结果给调用方，结束当前流程
                .filter(item -> item != null && idGetter.apply(item) != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
    ...
    private void notifyTenantsOfHouseOffline(House house, String content) { // 声明一个方法入口，约定参数和返回值以完成对应业务动作
        ...
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Order> wrapper = // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        wrapper.eq(Order::getHouseId, house.getId()) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
        ...
        Set<Long> tenantIds = orders.stream() // 创建集合变量，用于承载后续查询与组装的数据
                .map(Order::getTenantId) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .filter(id -> id != null) // 该行用于衔接当前业务步骤，保证整体流程顺序执行
                .collect(Collectors.toSet()); // 执行当前语句以推进业务流程，并为后续步骤准备数据
        tenantIds.forEach(tenantId -> // 该行用于衔接当前业务步骤，保证整体流程顺序执行
