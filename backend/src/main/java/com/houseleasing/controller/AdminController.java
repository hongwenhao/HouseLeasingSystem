...
public class AdminController { // 管理员专用控制器：用户、房源、订单、合同、统计等后台能力
    ...
    private static final String CREDIT_RANGE_EXCELLENT = "90-100(优秀)"; // 信用分优秀区间文案
    private static final String CREDIT_RANGE_GOOD = "70-89(良好)"; // 信用分良好区间文案
    private static final String CREDIT_RANGE_NORMAL = "60-69(一般)"; // 信用分一般区间文案
    private static final String CREDIT_RANGE_LOW = "60以下(较低)"; // 信用分较低区间文案
    /**
     * 后台订单状态白名单（统一使用大写枚举值）：
    ...
    public Result<PageResult<House>> listPendingHouses(
        ...
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<House> pageObj =
                new com.baomidou.mybatisplus.extension.plugins.pagination.Page<>(page, size); // 构建分页对象
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<House> wrapper =
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>(); // 构建查询条件
        wrapper.eq(House::getStatus, "OFFLINE"); // 待审核口径：仅查询 OFFLINE 房源
        if (StringUtils.hasText(keyword)) {
            wrapper.and(w -> w.like(House::getTitle, keyword)
                    .or().like(House::getCity, keyword)
                    .or().like(House::getAddress, keyword)); // 关键词匹配标题/城市/地址
        }
        wrapper.orderByDesc(House::getCreateTime); // 最新创建的房源优先展示
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<House> result = houseMapper.selectPage(pageObj, wrapper);
        return Result.success(PageResult.of(result.getTotal(), result.getRecords(), page, size)); // 返回分页结果
    }
    ...
    public Result<PageResult<House>> listAllHouses(
        ...
        if (StringUtils.hasText(keyword)) {
            wrapper.and(w -> w.like(House::getTitle, keyword)
                    .or().like(House::getCity, keyword)
                    .or().like(House::getAddress, keyword)); // 关键词模糊匹配房源基础信息
        }
        ...
        wrapper.orderByDesc(House::getCreateTime); // 按创建时间倒序
        com.baomidou.mybatisplus.extension.plugins.pagination.Page<House> result = houseMapper.selectPage(pageObj, wrapper);
        return Result.success(PageResult.of(result.getTotal(), result.getRecords(), page, size)); // 返回管理端房源分页数据
    }
    ...
    public Result<House> getHouseDetailForAdmin(@PathVariable Long id) {
        ...
        if (house == null) {
            throw new BusinessException(404, "房源不存在"); // 不存在时返回 404 业务异常
        }
        return Result.success(house); // 返回查询到的房源详情
    }
    ...
    public Result<Void> auditHouse(@PathVariable Long id,
        ...
        House house = houseMapper.selectById(id); // 查询待审核房源
        if (house == null) {
            throw new BusinessException(404, "房源不存在"); // 房源不存在直接拒绝
        }
        String status = body == null ? null : String.valueOf(body.getOrDefault("status", "")); // 读取审核状态字段
        if (!StringUtils.hasText(status)) {
            throw new BusinessException(400, "审核状态不能为空"); // 状态必填校验
        }
        boolean approved = "APPROVED".equalsIgnoreCase(status) || "ONLINE".equalsIgnoreCase(status); // 兼容 APPROVED/ONLINE 表达“通过”
        // 管理端房源管理口径仅保留 ONLINE/OFFLINE，两类下线语义统一写入 OFFLINE。
        house.setStatus(approved ? "ONLINE" : "OFFLINE"); // 审核通过上架，否则下架
        house.setUpdateTime(LocalDateTime.now()); // 刷新更新时间
        houseMapper.updateById(house); // 持久化审核结果
        // 审核动作后通知房东，便于房东及时感知房源状态变化
        sendHouseManagementNotificationToOwner(house, approved ? HOUSE_ACTION_ONLINE : HOUSE_ACTION_OFFLINE,
                approved ? OWNER_HOUSE_ONLINE_MESSAGE : OWNER_HOUSE_OFFLINE_MESSAGE); // 发送对应审核结果通知
    ...
    public Result<Void> putHouseOnline(@PathVariable Long id) {
        ...
        if (house == null) {
            throw new BusinessException(404, "房源不存在"); // 房源不存在时终止操作
        }
        house.setStatus("ONLINE"); // 将房源状态置为上架
        house.setUpdateTime(LocalDateTime.now()); // 更新操作时间
        houseMapper.updateById(house); // 写回数据库
        sendHouseManagementNotificationToOwner(house, HOUSE_ACTION_ONLINE, OWNER_HOUSE_ONLINE_MESSAGE); // 通知房东房源已上架
        return Result.success(); // 返回上架成功
    }
    ...
    public Result<Void> putHouseOffline(@PathVariable Long id) {
        House house = houseMapper.selectById(id); // 查询目标房源
        if (house == null) {
            throw new BusinessException(404, "房源不存在"); // 不存在时返回错误
        }
        house.setStatus("OFFLINE"); // 将房源状态置为下架
        house.setUpdateTime(LocalDateTime.now()); // 刷新更新时间
        houseMapper.updateById(house); // 持久化状态变更
        sendHouseManagementNotificationToOwner(house, HOUSE_ACTION_OFFLINE, OWNER_HOUSE_OFFLINE_MESSAGE); // 通知房东房源已下架
        // 对该房源产生过订单的租客推送提醒，避免租客继续对不可用房源发起意向
        notifyTenantsOfHouseOffline(house, String.format(TENANT_HOUSE_OFFLINE_MESSAGE_TEMPLATE, safeHouseTitle(house))); // 通知关联租客房源下架
        return Result.success(); // 返回下架成功
    }
    ...
    private User sanitizeUser(User source) {
        if (source == null) {
            return null; // 空对象直接返回，避免 NPE
        }
        User user = new User(); // 创建新对象，避免直接暴露原始实体全部字段
        user.setId(source.getId()); // 保留用户ID用于前端关联
        user.setUsername(source.getUsername()); // 返回用户名
        user.setPhone(source.getPhone()); // 返回手机号
        user.setEmail(source.getEmail()); // 返回邮箱
        user.setRole(source.getRole()); // 返回角色
        user.setRealName(source.getRealName()); // 返回实名信息（姓名）
        user.setCreditScore(source.getCreditScore()); // 返回信用分
        user.setIsRealNameAuth(source.getIsRealNameAuth()); // 返回实名认证状态
        user.setStatus(source.getStatus()); // 返回账号状态
        user.setAvatar(source.getAvatar()); // 返回头像
        return user; // 不包含密码等敏感字段
    }
    ...
    private BigDecimal parseDecimal(Object value) {
        if (value == null) {
            return BigDecimal.ZERO; // 空值按 0 处理，避免前端渲染异常
        }
        try {
            return new BigDecimal(String.valueOf(value)); // 统一用字符串转 BigDecimal，兼容多种输入类型
        } catch (Exception ignored) {
            return BigDecimal.ZERO; // 非法值兜底返回 0
        }
    }
    ...
    private long toLong(Object value) {
        ...
        if (value instanceof Number number) {
            return number.longValue(); // 数值类型直接转换
        }
        try {
            return Long.parseLong(String.valueOf(value)); // 字符串类型尝试解析
        } catch (Exception ignored) {
            return 0L; // 解析失败时兜底 0
        }
    }
    ...
    private <T> Map<Long, T> mapById(List<T> list, Function<T, Long> idGetter) {
        if (list == null || list.isEmpty()) {
            return Map.of(); // 空列表直接返回不可变空Map
        }
        return list.stream()
                .filter(item -> item != null && idGetter.apply(item) != null)
                .collect(Collectors.toMap(idGetter, Function.identity(), (a, b) -> a)); // 主键重复时保留首个元素
    }
    ...
    private void sendHouseManagementNotificationToOwner(House house, String actionLabel, String actionMessage) {
        if (house == null || house.getOwnerId() == null) {
            return; // 缺失房源或房东信息时不发送通知
        }
        String content = String.format("您的房源《%s》%s", safeHouseTitle(house), actionMessage); // 组装通知文案
        messageProducer.sendAdminHouseManagementNotification(house.getOwnerId(), actionLabel, content, house.getId()); // 异步推送给房东
    }
    ...
    private void notifyTenantsOfHouseOffline(House house, String content) {
        ...
        wrapper.eq(Order::getHouseId, house.getId())
                .select(Order::getTenantId); // 仅查询租客ID，降低查询开销
        List<Order> orders = orderMapper.selectList(wrapper); // 查询该房源关联订单
        Set<Long> tenantIds = orders.stream()
                .map(Order::getTenantId)
                .filter(id -> id != null)
                .collect(Collectors.toSet()); // 去重后得到需要通知的租客集合
        tenantIds.forEach(tenantId ->
                messageProducer.sendAdminHouseManagementNotification(tenantId, HOUSE_ACTION_OFFLINE, "房源状态变更提醒：" + content, house.getId())); // 对每位租客发送下架提醒
    }
    ...
    private String safeHouseTitle(House house) {
        if (house == null || !StringUtils.hasText(house.getTitle())) {
            return "未命名房源"; // 标题为空时使用兜底文案
        }
        return house.getTitle(); // 返回原始标题
    }
    ...
    private void notifyOrderCancelledByAdmin(Order order) {
        ...
        String orderNo = StringUtils.hasText(order.getOrderNo()) ? order.getOrderNo() : safeIdentifier(order.getId()); // 优先使用订单号，无则回退ID
        String message = String.format("管理员已取消订单（订单号：%s），如有疑问请联系平台客服。", orderNo); // 统一通知文案
    ...
    private void notifyContractCancelledByAdmin(Contract contract) {
        ...
        String contractNo = StringUtils.hasText(contract.getContractNo()) ? contract.getContractNo() : safeIdentifier(contract.getId()); // 优先合同编号，无则回退ID
        String message = String.format("管理员已取消合同（合同编号：%s），如有疑问请联系平台客服。", contractNo); // 统一通知文案
